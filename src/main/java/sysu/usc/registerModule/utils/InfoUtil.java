...
import sysu.usc.registerModule.VO.Web;
...
public class InfoUtil {
    ...
    public static <R> R getInfoFromDTO(FormDTO formDTO, Class<R> clazz) throws InstantiationException, IllegalAccessException {
        if(!clazz.getGenericSuperclass().getTypeName().equals(
                BaseObject.class.getTypeName())) {
            return null;
        }
        R instance = clazz.newInstance();
        BmcDTO bmcDTO = null;
        try {
            bmcDTO=queryFromBmc(formDTO.getId());
        } catch (WebDriverNotMatchException e) {
            e.printStackTrace();
        }
        //bmcDTO 内含学号姓名
        BeanUtil.copyProperties(bmcDTO, instance,
                                CopyOptions.create().ignoreError());
        return instance;
     }
     private static BmcDTO queryFromBmc(String id) throws WebDriverNotMatchException {
